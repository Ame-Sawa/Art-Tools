...
from cli_anything.blender.utils.blender_backend import find_blender, run_blender_script
...
def _create_smart_uv_fixture(path):
    script = "\n".join([
        "import bpy",
        "bpy.ops.wm.read_factory_settings(use_empty=True)",
        "root = bpy.data.objects.new('Root', None)",
        "bpy.context.scene.collection.objects.link(root)",
        "bpy.ops.mesh.primitive_cube_add(location=(1.25, -2.0, 3.0), rotation=(0.2, 0.4, -0.3), scale=(-1.5, 0.75, 2.0))",
        "mesh_a = bpy.context.object; mesh_a.name = 'Mesh_A'; mesh_a.parent = root; mesh_a.matrix_parent_inverse = root.matrix_world.inverted()",
        "material = bpy.data.materials.new('Mat_A'); mesh_a.data.materials.append(material)",
        "bpy.ops.mesh.primitive_uv_sphere_add(segments=16, ring_count=8, location=(-2.0, 1.5, 0.5), rotation=(0.3, -0.2, 0.5), scale=(0.5, 1.25, 0.75))",
        "mesh_b = bpy.context.object; mesh_b.name = 'Mesh_B'; mesh_b.parent = mesh_a; mesh_b.matrix_parent_inverse = mesh_a.matrix_world.inverted(); mesh_b.data.materials.append(material)",
        "mesh_a.keyframe_insert(data_path='rotation_euler', frame=1); mesh_a.rotation_euler.z += 0.5; mesh_a.keyframe_insert(data_path='rotation_euler', frame=20)",
        "camera_data = bpy.data.cameras.new('CameraData'); camera = bpy.data.objects.new('Camera', camera_data); bpy.context.scene.collection.objects.link(camera); camera.parent = root",
        "light_data = bpy.data.lights.new('LightData', 'POINT'); light = bpy.data.objects.new('Light', light_data); bpy.context.scene.collection.objects.link(light); light.parent = root",
        f"bpy.ops.export_scene.fbx(filepath={path!r}, check_existing=False, use_selection=False, use_visible=False, use_active_collection=False, object_types={{'EMPTY', 'CAMERA', 'LIGHT', 'ARMATURE', 'MESH', 'OTHER'}}, use_mesh_modifiers=False, add_leaf_bones=False, bake_anim=True, axis_forward='-Z', axis_up='Y')",
    ])
    result = run_blender_script(script, timeout=120)
...
class TestFbxSmartUvE2E:
    def test_cli_smart_uv_preserves_roundtrip_semantics(self, tmp_dir):
        ...
        result = subprocess.run(
            TestCLISubprocess.CLI_BASE + ["--json", "fbx", "smart-uv-project", source],
            capture_output=True,
            text=True,
            timeout=240,
        )
        ...
        output_path = os.path.join(tmp_dir, "hierarchy_uv.fbx")
        assert payload["output_fbx"] == os.path.abspath(output_path)
        ...
        assert payload["mesh_objects"] == 2
        assert payload["uv_loop_count"] > 0
        with open(source, "rb") as fh:
            assert fh.read() == source_before

    def test_overwrite_source_requires_explicit_flag(self, tmp_dir):
        source = os.path.join(tmp_dir, "source.fbx")
        _create_smart_uv_fixture(source)
        with pytest.raises(ValueError, match="source FBX"):
            export_fbx_smart_uv(source, source)
