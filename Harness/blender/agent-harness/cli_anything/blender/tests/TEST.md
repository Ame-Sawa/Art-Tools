...
|------|-------------|------------|-------|
| `test_core.py` | 9 | 160 | Unit tests for scene, objects, materials, modifiers, lighting, animation, render, FBX, session |
...
### TestFbxRender (4 tests)
- Generate syntactically valid standalone Blender Python for FBX import and material-slot coloring
- Reject an existing output path unless overwrite is explicitly enabled
- Generate syntactically valid multi-angle FBX render scripts
- Reject unsupported multi-angle view names before Blender is launched

### TestFbxSmartUv
- Generate syntactically valid Smart UV import/export and validation scripts
- Validate Smart UV parameters and output/source overwrite guards
- Validate Blender round-trip hierarchy, transforms, handedness, animation, mesh structure, and UV presence
