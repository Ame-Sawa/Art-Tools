...
class TestFbxRender:
    def test_generated_fbx_script_is_valid_python(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        fbx_path.write_bytes(b"placeholder")
        output_path = tmp_path / "render.png"
        script = generate_fbx_render_script(
            str(fbx_path), str(output_path), color_by_material=True,
            engine="EEVEE", resolution_x=800, resolution_y=600,
        )
        compile(script, "<fbx_render_script>", "exec")
        assert "bpy.ops.import_scene.fbx" in script
        assert "CLI_MaterialColor" in script
        assert "CLI_DefaultGray" in script
        assert "has_missing_image" in script
        assert output_path.name in script

    def test_rejects_existing_output_without_overwrite(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        output_path = tmp_path / "render.png"
        fbx_path.write_bytes(b"placeholder")
        output_path.write_bytes(b"existing")
        with pytest.raises(FileExistsError):
            render_fbx(str(fbx_path), str(output_path), timeout=1)

    def test_generated_multi_angle_script_is_valid_python(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        fbx_path.write_bytes(b"placeholder")
        output_paths = {
            "front": str(tmp_path / "front.png"),
            "top": str(tmp_path / "top.png"),
        }
        script = generate_fbx_render_script(
            str(fbx_path), output_paths["front"], output_paths=output_paths,
        )
        compile(script, "<multi_angle_fbx_render_script>", "exec")
        assert "VIEW_OFFSETS" in script
        assert "for view_name, output_path" in script

    def test_multi_angle_rejects_unknown_view(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        fbx_path.write_bytes(b"placeholder")
...
class TestFbxSmartUv:
    def test_generated_smart_uv_script_is_valid_python(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        output_path = tmp_path / "model_uv.fbx"
        script = generate_fbx_smart_uv_script(str(fbx_path), str(output_path))
        compile(script, "<fbx_smart_uv_script>", "exec")
        assert "bpy.ops.import_scene.fbx" in script
        assert "bpy.ops.uv.smart_project" in script
        assert "bpy.ops.export_scene.fbx" in script
        assert "RIGHT_HAND_AXES" in script
        assert "object_types={'EMPTY', 'CAMERA', 'LIGHT', 'ARMATURE', 'MESH', 'OTHER'}" in script
        assert output_path.name in script

    def test_generated_validation_script_is_valid_python(self, tmp_path):
        script = generate_fbx_smart_uv_validation_script(
            str(tmp_path / "source.fbx"), str(tmp_path / "output.fbx")
        )
        compile(script, "<fbx_smart_uv_validation_script>", "exec")
        assert "FBX_SMART_UV_VALIDATION" in script
        assert "determinant_sign" in script
        assert "matrix_world" in script

    def test_smart_uv_options_are_validated(self, tmp_path):
        with pytest.raises(ValueError, match="angle_limit"):
            generate_fbx_smart_uv_script(
                str(tmp_path / "source.fbx"), str(tmp_path / "output.fbx"),
                smart_uv_options={"angle_limit": 4.0},
            )
        with pytest.raises(ValueError, match="area_weight"):
            generate_fbx_smart_uv_script(
                str(tmp_path / "source.fbx"), str(tmp_path / "output.fbx"),
                smart_uv_options={"area_weight": 2.0},
            )

    def test_smart_uv_rejects_source_overwrite_without_explicit_flag(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        fbx_path.write_bytes(b"placeholder")
        with pytest.raises(ValueError, match="source FBX"):
            export_fbx_smart_uv(str(fbx_path), str(fbx_path))

    def test_smart_uv_rejects_existing_output_without_overwrite(self, tmp_path):
        fbx_path = tmp_path / "model.fbx"
        output_path = tmp_path / "model_uv.fbx"
        fbx_path.write_bytes(b"placeholder")
        output_path.write_bytes(b"existing")
