...
def _update_current_symlink(session_dir: Path, bundle_dir: str) -> Path:
    ...
    try:
        ...
        # Windows without symlink privilege can still work by materializing
        # a `current/` snapshot of the latest bundle.
        shutil.copytree(Path(bundle_dir).resolve(), current_link)
    return current_link
