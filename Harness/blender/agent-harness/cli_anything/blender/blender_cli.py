...
# Add the harness root to sys.path so both `python -m ...` and direct script
# execution can resolve the top-level `cli_anything` package.
...
@cli.group("fbx")
def fbx_group():
    """Import, process, export, and render FBX files."""
    pass


def _fbx_render_options(command):
    command = click.option("--engine", type=click.Choice(["CYCLES", "EEVEE", "WORKBENCH"]), default="EEVEE", show_default=True)(command)
    command = click.option("--resolution-x", "resolution_x", type=int, default=1920, show_default=True)(command)
    command = click.option("--resolution-y", "resolution_y", type=int, default=1080, show_default=True)(command)
    command = click.option("--samples", type=int, default=64, show_default=True)(command)
    command = click.option("--transparent", is_flag=True, help="Render with transparent background")(command)
    command = click.option("--overwrite", is_flag=True, help="Allow replacing an existing output file")(command)
    command = click.option("--timeout", type=int, default=300, show_default=True, help="Blender timeout in seconds")(command)
    return command


@fbx_group.command("render")
@click.argument("fbx_path", type=click.Path(exists=True, dir_okay=False))
@click.argument("output_path", type=click.Path(dir_okay=False))
@_fbx_render_options
@handle_error
def fbx_render(fbx_path, output_path, engine, resolution_x, resolution_y, samples,
               transparent, overwrite, timeout):
    """Render FBX_PATH in an empty scene to OUTPUT_PATH, preserving materials."""
    result = fbx_mod.render_fbx(
        fbx_path, output_path, engine=engine, resolution_x=resolution_x,
        resolution_y=resolution_y, samples=samples, transparent=transparent,
        overwrite=overwrite, timeout=timeout,
    )
    output(result, f"Rendered FBX: {result['output']}")


@fbx_group.command("material-colors")
@click.argument("fbx_path", type=click.Path(exists=True, dir_okay=False))
@click.argument("output_path", type=click.Path(dir_okay=False))
@_fbx_render_options
@handle_error
def fbx_material_colors(fbx_path, output_path, engine, resolution_x, resolution_y, samples,
                        transparent, overwrite, timeout):
    """Render FBX_PATH with a distinct flat color per mesh material slot."""
    result = fbx_mod.render_fbx(
        fbx_path, output_path, color_by_material=True, engine=engine,
        resolution_x=resolution_x, resolution_y=resolution_y, samples=samples,
        transparent=transparent, overwrite=overwrite, timeout=timeout,
    )
    output(result, f"Rendered colorized FBX: {result['output']}")


@fbx_group.command("multi-angle")
@click.argument("fbx_path", type=click.Path(exists=True, dir_okay=False))
@click.argument("output_dir", type=click.Path(file_okay=False))
@click.option("--view", "views", multiple=True,
              type=click.Choice(fbx_mod.MULTI_ANGLE_VIEWS),
              help="View to render; repeat this option. Defaults to front, right, top, and perspective.")
@click.option("--format", "image_format", default="png", show_default=True,
              type=click.Choice(["png", "jpg", "jpeg", "bmp", "tif", "tiff", "exr"], case_sensitive=False))
@click.option("--material-colors", is_flag=True,
              help="Assign a distinct diagnostic color to every mesh material slot")
@_fbx_render_options
@handle_error
def fbx_multi_angle(fbx_path, output_dir, views, image_format, material_colors, engine,
                    resolution_x, resolution_y, samples, transparent, overwrite, timeout):
    """Render FBX_PATH from several views into OUTPUT_DIR in one Blender run."""
    result = fbx_mod.render_fbx_multi_angle(
        fbx_path, output_dir, views=views, extension=image_format,
        color_by_material=material_colors, engine=engine, resolution_x=resolution_x,
        resolution_y=resolution_y, samples=samples, transparent=transparent,
        overwrite=overwrite, timeout=timeout,
    )
    output(result, f"Rendered {len(result['outputs'])} FBX views to: {result['output_dir']}")


@fbx_group.command("smart-uv-project")
@click.argument("fbx_path", type=click.Path(exists=True, dir_okay=False))
@click.option("--output", "output_path", type=click.Path(dir_okay=False), default=None,
              help="Explicit output FBX path; defaults to INPUT with a _uv suffix.")
...
@click.option("--angle-limit", type=float, default=None,
              help="Smart UV angle limit in radians.")
@click.option("--margin-method", type=click.Choice(fbx_mod.SMART_UV_MARGIN_METHODS), default=None,
              help="Smart UV island margin method.")
@click.option("--rotate-method", type=click.Choice(fbx_mod.SMART_UV_ROTATE_METHODS), default=None,
              help="Smart UV island rotation method.")
@click.option("--island-margin", type=float, default=None,
              help="Smart UV island margin value.")
@click.option("--area-weight", type=float, default=None,
              help="Smart UV area weighting from 0 to 1.")
@click.option("--correct-aspect/--no-correct-aspect", default=None,
              help="Correct UV aspect ratio during projection.")
@click.option("--scale-to-bounds/--no-scale-to-bounds", default=None,
